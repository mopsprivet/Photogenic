...
class SearchPhotoViewController: UIViewController, UICollectionViewDataSource, UISearchBarDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        collectionView?.frame = CGRect(x: 0, y: Int(view.safeAreaInsets.top), width: Int(view.frame.size.width), height: Int(view.frame.size.height))
        ...
        let task = URLSession.shared.dataTask(with: url) { [weak self] data, _, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    print("Updated results count:", self?.results.count ?? 0)  // Проверить количество
                    self?.collectionView?.reloadData()  // Вызвать reloadData после обновления результатов
                }
            } catch {
